...
\zexternaldocument{author_ms}  % other .tex file to cross reference
...
\title{Supporting Information: APA 6th-style research reports with LaTeX{}: A not-especially
  tongue-in-cheek reproduced reproducible research report report}
...
Figure~\zref{lp_filt} in the main document. The reference is via the
...
If you already use conda environments in a recent linux or OSX
operation system, you can install a minimal environment to run the
notebooks used for this report like so. 

\begin{minted}{bash}
conda create -n apa6th_report python numpy pandas pyarrow scipy matplotlib jupyter
$ conda create -n apa67_report numpy pandas matplotlib jupyter spudtr -c kutaslab -c defaults -c conda-forge
$ activate apa67_report
$ jupyter notebooks
\end{minted}

If you are not yet set up to use conda environments, you can follow the
instructions to download and install a minimal conda installer,
miniconda3. This provides just enough infrastructure to create a conda
environemnt and install packages as shown in the example above.
...
run pipeline\_2.ipynb. It is available via pip install, but there is no
assurance it is compatible with the versions of packages you already
have installed.
...
\includepdf[pages={1-}]{author_analysis}


% ------------------------------------------------------------
% research report LaTeX
\newpage
\section{Source: {\tt research\_report.tex}}\zlabel{author_ms_tex}
...
\newpage
\section{Source: {\tt author\_si.tex}}\zlabel{author_si_tex}
...
\inputminted{latex}{author_fig1.tex}
...
\inputminted{latex}{author_fig2.tex}
...
\inputminted{bibtex}{author_ms.bib}
